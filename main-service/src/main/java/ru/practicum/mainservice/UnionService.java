...
public class UnionService {
    ...
    public User getUserOrNotFound(Long userId) {
        ...
        return userRepository.findById(userId).orElseThrow(() -> new NotFoundException(User.class, "Пользователь с id " + userId + " не найден."));

    }

    public Category getCategoryOrNotFound(Long categoryId) {
        return categoryRepository.findById(categoryId).orElseThrow(() -> new NotFoundException(Category.class, "Категория с id " + categoryId + " не найдена."));
    ...
    public Request getRequestOrNotFound(Long requestId) {
        ...
        return requestRepository.findById(requestId).orElseThrow(() -> new NotFoundException(Request.class, "Запрос с id " + requestId + " не найден."));

    }

    public Compilation getCompilationOrNotFound(Long compId) {

        return compilationRepository.findById(compId).orElseThrow(() -> new NotFoundException(Compilation.class, "Список событий с  id " + compId + " not found."));

    }

    public Comment getCommentOrNotFound(Long commentId) {
        return commentRepository.findById(commentId).orElseThrow(() -> new NotFoundException(Comment.class, "Комменатрий с  id " + commentId + " не найден."));
    }
